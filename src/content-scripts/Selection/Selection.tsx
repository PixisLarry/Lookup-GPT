...
const [SelectionProvider, useSelectionContext] =
    createContext<SelectionContextValue>('Selection')
...
type SelectionTriggerProps = SelectionTriggerImplProps

const SelectionTrigger = React.forwardRef<
...
type SelectionTriggerWhileSelectProps = SelectionTriggerImplProps
    ...
    React.useEffect(() => {
        if (!context.disabled) {
            const handleSelection = () => {
                ...
                const wasSelectionInsideTrigger = node?.contains(
...
type SelectionTriggerImplElement = React.ElementRef<any>
type PrimitiveDivProps = React.ComponentPropsWithoutRef<any>
type SelectionTriggerImplProps = PrimitiveDivProps
...
const [PortalProvider, usePortalContext] = createContext<PortalContextValue>(
    PORTAL_NAME,
    {
...
const SelectionContent = React.forwardRef<
...
type SelectionContentImplElement = React.ElementRef<any>
...
>((props, forwardedRef) => {
    ...
    const { x, y, strategy, placement, refs, middlewareData, isPositioned } =
        useFloating({
            strategy: 'fixed',
        ...
        <div
            ...
            style={{
                ...
                transform: isPositioned
                    ? `translate3d(${Math.round(x ?? 0)}px, ${Math.round(
                          y ?? 0
                      )}px, 0)`
                    : 'translate3d(0, -200%, 0)',
            ...
            >
                <div
                    ...
                    style={{
                        ...
                        opacity: middlewareData.hide?.referenceHidden
                            ? 0
                            : undefined,
...
type SelectionArrowProps = ArrowProps

const SelectionArrow = React.forwardRef<
    SelectionArrowElement,
    SelectionArrowProps
>((props, forwardedRef) => {
...
const anchorCssProperties = (): Middleware => ({
    ...
    async fn(data) {
        ...
        elements.floating.style.setProperty(
            '--selection-popover-select-width',
            `${width}px`
        )
        elements.floating.style.setProperty(
            '--selection-popover-select-height',
            `${height}px`
        )
        const newDimensions = await platform.getDimensions(elements.floating)
        if (
            popoverWidth !== newDimensions.width ||
            popoverHeight !== newDimensions.height
        ) {
